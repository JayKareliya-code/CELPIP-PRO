...
async def patch_asset(asset_id: uuid.UUID, body: AssetPatch, db: DB, _: Admin) -> dict[str, Any]:
    repo = AdminAssetRepo(db)
    asset = await repo.get_by_id(asset_id)
    if not asset:
        raise HTTPException(404, "Asset not found")
    updated = await repo.update(asset, **body.model_dump(exclude_none=True))
    await db.commit()
...
async def remove_prompt_image(
    prompt_id: uuid.UUID, image_id: uuid.UUID, db: DB, _: Admin  # noqa: ARG001
) -> None:
    repo = AdminAssetRepo(db)
